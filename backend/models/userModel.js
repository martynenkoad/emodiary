...
const { onlyLetters, replaceSpaces } = require('../lib/string-routines')
...
userSchema.statics.signup = async function (firstName, lastName, login, password) {
  ...
  const nameWoutSpaces = replaceSpaces(firstName)
  const surnameWoutSpaces = replaceSpaces(lastName)
  const loginWoutSpaces = replaceSpaces(login)
  ...
  const user = await this.create({
    firstName: nameWoutSpaces,
    lastName: surnameWoutSpaces,
    login: loginWoutSpaces,
    password: hash
  })
...
userSchema.statics.logIn = async function (login, password) {
  ...
  if (!user) {
    throw Error('Oops you are not the emo yet! :( Don\'t worry - you can sign up any time you want! <3')
  }
